 ...
 * init method from here, but this is more general solution because @PostContextUp can be used everywhere in project.
 */
@Component
public class PostProxyInvokerContextListener implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        ...
        for (String beanDefinitionName : beanDefinitionNames) {
            // Todo? There are some errors when originalBeanClassName is null or some beans from servlet context are not
            // found. Maybe avoid this universal logic and inject here persistence and run init from here
            try {
                ...
                for (Method method : originalClass.getMethods()) {
                    if (method.isAnnotationPresent(PostContextUp.class)) {
